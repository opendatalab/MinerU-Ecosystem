...
func init() {
	...
	extractCmd.Flags().BoolVar(&extractOCR, "ocr", false, "OCR for scanned documents (default off)")
	extractCmd.Flags().BoolVar(&extractFormula, "formula", true, "Formula recognition (default on, use --formula=false to disable)")
	extractCmd.Flags().BoolVar(&extractTable, "table", true, "Table recognition (default on, use --table=false to disable)")
